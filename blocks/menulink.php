...
function a_menulink_show($options)
{
    ...
    $result = $xoopsDB->query('SELECT groups, link, title, target FROM ' . $xoopsDB->prefix('ev_menulink') . ' WHERE hide=0 ORDER BY weight ASC');
...
{
    $form = _BM_EV_MENULINK_CHARS . "&nbsp;<input type='text' name='options[]' value='" . $options[0] . "'>&nbsp;" . _BM_EV_MENULINK_LENGTH . '';
